...
\begin{itemize}%
\item%
\textit{Gradient Descent for Spiking Neural Networks}%
\newline%
Read date: 2018{-}09{-}24%
\newline%
\href{https://arxiv.org/pdf/1706.04698.pdf}{https://arxiv.org/pdf/1706.04698.pdf}%
\newline%
Synopsis: Most studies of neural computation are based on network models of rate{-}based neurons that process analog input, despite the fact that information processing in the brain is predominantly done by dynamic neurons that produce spikes. Research in spike{-}based computation is impeded by the lack of an efficient supervised learning algorithm for spiking neural networks (SNNs). The authors present a gradient descent method for optimizing SNN models by introducing a differentiable formulation of SNNs and deriving the exact gradient calculation. They train recurrent SNNs for two dynamic tasks: one requiring millisecond time scale spike{-}based interactions for efficient information encoding, and a delayed{-}memory XOR task over extended time (on the order of seconds). The method is able to optimize SNN dynamics on the time scale of individual spikes as well as behavior. The result is a general purpose supervised learning algorithm for SNNs.%
\item%
\textit{Supervised learning in spiking neural networks with FORCE training}%
\newline%
Read date: 2018{-}09{-}23%
\newline%
\href{https://www.nature.com/articles/s41467{-}017{-}01827{-}3.pdf}{https://www.nature.com/articles/s41467{-}017{-}01827{-}3.pdf}%
\newline%
Synopsis: FORCE training, previously introduced to trained networks of rate{-}based neurons, is used to train spiking neural networks (SNNs) to mimic dynamical systems, classify inputs, store discrete sequences that correspond to notes in a song, and mimic two biologically motivated model circuits. The first is used to reproduce songbird singing in the adult zebra finch, and the second is trained to store and replay a movie scene, which requires a high{-}dimensional temporal signal (HDTS), jointly trained or fixed, in order to successfully learn. FORCE{-}trained networks reproduce behaviors comparable in complexity to those of their biologically inspired circuits and yield information not easily obtainable with methods such as pharmacological manipulations or spike time statistics.%
\item%
...
Read date: 2018{-}04{-}01%
\newline%
\href{https://papers.nips.cc/paper/2307{-}a{-}model{-}for{-}real{-}time{-}computation{-}in{-}generic{-}neural{-}microcircuits.pdf}{https://papers.nips.cc/paper/2307{-}a{-}model{-}for{-}real{-}time{-}computation{-}in{-}generic{-}neural{-}microcircuits.pdf}%
\newline%
